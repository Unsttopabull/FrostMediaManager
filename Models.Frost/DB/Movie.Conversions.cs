    ...
    public partial class Movie
    {
        ...
        public static explicit operator Movie(XjbXmlMovie xm) {
            ...
            Movie mv = new Movie {
                ...
                Certifications = new HashSet<Certification>(Certification.ParseCertificationsString(xm.CertificationsString)),
                ImdbID = xm.ImdbId,
            ...
            mv.Genres = new HashSet<Genre>(Genre.GetFromNames(xm.Genres));

            //Convert and Add XjbXmlActor array to HashSet<Actor>
            if (xm.Actors != null) {
                foreach (XjbXmlActor actor in xm.Actors) {
                    mv.Actors.Add(new Actor(actor.Name, actor.Thumb, actor.Role));
                }
            }
        ...
        public static explicit operator Movie(XbmcXmlMovie mx) {
            Movie mv = new Movie {
                ...
                Watched = mx.Watched,
                ReleaseYear = mx.Year,
                Art = new HashSet<Art>(GetArt(mx)),
                Directors = new HashSet<Person>(from d in mx.Directors
                                                          where !string.IsNullOrEmpty(d)
                                                          select new Person(d)),

                Writers = new HashSet<Person>(from c in mx.Credits
            ...
            if (mx.Certifications != null) {
                mv.Certifications = new HashSet<Certification>(mx.Certifications.Select(xmlCert => (Certification) xmlCert));
            }

            if (mx.FileInfo.InfoExists(MediaType.Audio)) {
                mv.Audios = new HashSet<Audio>(mx.FileInfo.Audios.Select(xmlAudio => (Audio) xmlAudio));
                
            }

            if (mx.FileInfo.InfoExists(MediaType.Subtitles)) {
                mv.Subtitles = new HashSet<Subtitle>(mx.FileInfo.Subtitles.Select(xmlSub => (Subtitle) xmlSub));
            }
            
            if(mx.FileInfo.InfoExists(MediaType.Video)){
                mv.Videos = new HashSet<Video>(mx.FileInfo.Videos.Select(xmlVideo => (Video) xmlVideo));
            }

            mv.Actors = new HashSet<Actor>(mx.Actors.Select(a => new Actor(a.Name, a.Thumb, a.Role)));
            mv.Plots.Add(new Plot(mx.Plot, mx.Outline, mx.Tagline, null));

            return mv;
        }

        private static IEnumerable<Art> GetArt(XbmcXmlMovie mx) {
            ...
            if (mx.Thumbs != null) {
                ...
                foreach (XbmcXmlThumb thumb in mx.Thumbs) {
                    ...
                    if (string.IsNullOrEmpty(thumb.Aspect)) {
                        a = new Art(ArtType.Unknown, thumb.Path, thumb.Preview);
                    }
                    else {
                        switch (thumb.Aspect.ToLower()) {
                            case "poster":
                                a = new Art(ArtType.Poster, thumb.Path, thumb.Preview);
                                break;
                            case "cover":
                                a = new Art(ArtType.Cover, thumb.Path, thumb.Preview);
                                break;
                            default:
            ...
            if (mx.Fanart != null && mx.Fanart.Thumbs != null) {
                //add fanart
                art.AddRange(mx.Fanart.Thumbs.Select(thumb => new Art(ArtType.Fanart, thumb.Path, thumb.Preview)));
            }
            return art;
        }
